...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("processing_international.log", mode='a', encoding='utf-8'), # Изменено имя лог файла
...
KNOWLEDGE_MAP_PATH = "knowledge_map_international.json" # Изменено имя файла карты
TEMP_DIR = "temp_international" # Изменена папка temp
LAST_PROCESSED_FILE = "last_processed_international.txt" # Изменено имя файла
TEMP_RESULTS_FILE = "results/international_events_temp.json" # Изменено имя файла
FINAL_RESULTS_FILE = "results/international_events.json" # Изменено имя файла
...
# -----------------------------------------------------------------------------
# ЗАГРУЗКА И ФОРМАТИРОВАНИЕ КАРТЫ ЗНАНИЙ (остается без изменений)
# -----------------------------------------------------------------------------
def format_knowledge_node_for_prompt(node: Dict[str, Any], indent_level: int = 0) -> str:
    ...
    # Убрал node_type_prefix для более чистого вида карты в промпте, т.к. структура и так понятна
...
def load_and_format_knowledge_map(file_path: str) -> str:
    try:
        ...
        if not isinstance(structured_map, list): # Ожидаем список категорий верхнего уровня
...
# -----------------------------------------------------------------------------
# СИСТЕМНЫЕ ПРОМПТЫ ДЛЯ МОДЕЛЕЙ - ИЗМЕНЕНЫ
# -----------------------------------------------------------------------------
...
"""
# -----------------------------------------------------------------------------
# МОДЕЛЬ ДАННЫХ (PYDANTIC) - ИЗМЕНЕНА
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# УТИЛИТЫ ДЛЯ РАБОТЫ С API (остаются без изменений)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# УТИЛИТЫ ДЛЯ ОБРАБОТКИ ДАННЫХ - АДАПТИРОВАНА ensure_default_values
# -----------------------------------------------------------------------------
def ensure_default_values_international(event_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for field, valid_values in literal_fields_map.items():
        ...
        if current_value is not None and current_value not in valid_values:
            ...
            event_dict[field] = "Medium" # Установка в Medium, а не None, т.к. это поле уверенности
    return event_dict

def load_diary_data(file_path: str) -> pd.DataFrame: # Остается без изменений
    try:
...
def extract_international_events(entry_id: int, text: str, date: str, extractor_model, current_knowledge_map_str: str) -> List[Dict[str, Any]]:
    ...
    **ЗАДАЧА:**
    ...
    НЕ ИЗВЛЕКАЙ: внутреннюю политику отдельных стран (если она не имеет прямого международного резонанса, например, не приводит к войне или интервенции), общественные настроения внутри одной страны, личное восприятие или эмоции автора, экономические или культурные события локального значения. Фокусируйся на фактах событий на международной арене.
    ...
    5.  `location`: Место события из текста. Если неясно, "Не указано".
    6.  `location_normalized`: Нормализованное место (город/страна) НА РУССКОМ. Если неясно, `null`.
    7.  `brief_context`: Краткий (1-2 предложения) КОНКРЕТНЫЙ исторический факт, поясняющий международное значение события.
    8.  `keywords`: Список из 3-5 ключевых слов/фраз, характеризующих международное событие.
    9.  `text_fragment`: ТОЧНАЯ цитата (ОДНО или НЕСКОЛЬКО ПОЛНЫХ ПРЕДЛОЖЕНИЙ) для контекста международного события.
    10. `confidence_extraction`: Оцени общую уверенность ("High", "Medium", "Low") в корректности извлеченных данных для этого события.
        ...
        try:
            response = extractor_model.generate_content(
                ...
                generation_config=GenerationConfig(
                    temperature=0.2, # Снижаем температуру для более точного следования инструкциям
...
def verify_international_event(event_data: Dict[str, Any], verifier_model, full_text: str, current_knowledge_map_str: str) -> Dict[str, Any]:
    ...
    1.  **Релевантность и Классификация:**
        *   Убедись, что событие является МЕЖДУНАРОДНЫМ и относится к 1848-1849 гг. Если нет, можешь вернуть пустой объект `{{}}` или объект только с `entry_id` и `source_date` и `event_id` = `null`, чтобы указать на нерелевантность.
    ...
    **Формат ответа:**
    Верни исправленную версию события ТОЛЬКО в формате JSON объекта. Если событие нерелевантно (не международное), верни пустой объект `{{}}`.
    """
    ...
    while retry_count < MAX_RETRIES:
        try:
            response = verifier_model.generate_content(
                ...
                safety_settings=SAFETY_SETTINGS,
                generation_config=GenerationConfig(
                    temperature=0.3, # Верификатор тоже должен быть точным
                    response_mime_type="application/json"
                )
            )
            if response.prompt_feedback and response.prompt_feedback.block_reason:
            ...
            if not response.candidates:
                # ... (обработка отсутствия кандидатов как раньше)
            ...
            if not verified_event: # Если верификатор вернул пустой объект {}
                logger.info(f"Верификатор (международные) счел событие из entry_id {event_data.get('entry_id')} нерелевантным.")
                return {} # Возвращаем пустой словарь, чтобы его можно было отфильтровать

            if not isinstance(verified_event, dict):
                # ... (обработка ошибки типа как раньше)
            ...
            # Убедимся, что ключевые поля присутствуют, если объект не пустой
            ...
            # ... (обработка ошибки декодирования как раньше)
...
# -----------------------------------------------------------------------------
# ОСНОВНАЯ ФУНКЦИЯ ОБРАБОТКИ ДНЕВНИКА - АДАПТИРОВАНА
# -----------------------------------------------------------------------------
def process_diary():
    """Основной конвейер обработки дневниковых записей для извлечения МЕЖДУНАРОДНЫХ событий."""
    ...
    for index, row in data.iterrows():
        ...
        temp_file_path = os.path.join(TEMP_DIR, f"entry_international_{current_entry_id}.json") # Изменено имя temp-файла
        ...
        if os.path.exists(temp_file_path):
            ...
            try:
                ...
                for evt_data in entry_events_from_temp:
                    if isinstance(evt_data, dict) and evt_data: # Проверяем, что объект не пустой (если верификатор вернул {})
                        ...
                        if key not in existing_event_keys:
                            try:
                                ...
                                # Используем новую Pydantic модель и ensure_default_values
        ...
        try:
            ...
            for event_item_data in extracted_events_data:
                ...
                if not isinstance(verified_event_item_data, dict): # Должно быть обработано в verify_international_event, но проверим
                    logger.warning(f"Верификатор (международные) вернул не-словарь для {current_entry_id}: {verified_event_item_data}.")
                    continue

                if not verified_event_item_data: # Если верификатор вернул пустой словарь (нерелевантно)
                    continue
                ...
                try:
                    validated_event = InternationalEvent(**verified_event_item_data) # Используем новую Pydantic модель
