# ПОСЛЕДНЯЯ УСТОЙЧИВАЯ ВЕРСЯ НА 1=х КЛИЕНТЕ для FORGET API
...
# -----------------------------------------------------------------------------
# ИЗМЕНЕНО: Добавлен раздел "ВАЖНОЕ ОГРАНИЧЕНИЕ" для фильтрации литературных обсуждений.
...
**ВАЖНОЕ ОГРАНИЧЕНИЕ: Литература против Истории**
Игнорируй и не извлекай информацию, которая является исключительно обсуждением или пересказом содержания прочитанных автором книг, литературных произведений или пьес. Извлекай событие, только если автор обсуждает реальные исторические события, даже если он узнал о них из газет, которые можно считать печатной продукцией.
*   **Пример разграничения:** Обсуждение сюжета романа "Граф Монте-Кристо" — это ЛИТЕРАТУРА (игнорировать). Обсуждение новости из газеты о боях в Венгрии — это ИСТОРИЧЕСКОЕ СОБЫТИЕ (извлекать).
...
def extract_revolution_events(entry_id: int, text: str, date: str, client: OpenAI, current_knowledge_map_str: str) -> List[Dict[str, Any]]:
    ...
    *Пример 2: Слухи и личное восприятие автора*
    ...
    """
    # ИЗМЕНЕНО: Добавлены/усилены инструкции по фильтрации литературных обсуждений.
    user_prompt = f"""
    ...
    **КАРТА ЗНАНИЙ ДЛЯ КЛАССИФИКАЦИИ:**
    ...
    ---
    **ЗАДАЧА:**
    Внимательно прочитай текст дневниковой записи. Извлеки из него ВСЕ упоминания, ПРЯМО связанные с реальными историческими событиями революций 1848-1849 гг. в Европе, их последствиями, а также реакцией и восприятием этих событий автором дневника. Для КАЖДОГО такого найденного упоминания (события/аспекта) сформируй JSON-объект со следующими полями:
    ...
    *   Извлекай ТОЛЬКО упоминания, которые ПРЯМО связаны с реальными историческими революциями 1848-1849 гг. в Европе.
    *   **Игнорируй литературные обсуждения:** Не извлекай информацию, если она является пересказом сюжета или обсуждением персонажей книги, пьесы или другого художественного произведения. Сосредоточься только на реальных исторических событиях, новостях, слухах и политических дискуссиях.

    **ДОПОЛНИТЕЛЬНАЯ ПРОВЕРКА:**
    Перед включением каждого упоминания задай себе вопрос: "Можно ли это упоминание ПРЯМО связать с реальными революционными событиями 1848-1849 гг. в Европе, а не с сюжетом художественного произведения?" Если ответ "нет" или "возможно" - НЕ включай.
