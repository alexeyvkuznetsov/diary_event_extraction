...
# --- ВЕРСИЯ С ПОСТ-ОБРАБОТКОЙ ДЛЯ УДАЛЕНИЯ <think> ТЕГОВ ---
...
""" # Убрана инструкция "Предоставь только конечный ответ... без использования тегов <think>"
...
# -----------------------------------------------------------------------------
# УТИЛИТЫ ДЛЯ РАБОТЫ С API - без существенных изменений
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# УТИЛИТЫ ДЛЯ ОБРАБОТКИ ДАННЫХ (ensure_default_values без изменений)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# ФУНКЦИЯ ИЗВЛЕЧЕНИЯ ДАННЫХ (С ПОСТ-ОБРАБОТКОЙ)
# -----------------------------------------------------------------------------
def extract_revolution_events(entry_id: int, text: str, date: str, client: OpenAI, current_knowledge_map_str: str) -> List[Dict[str, Any]]:
    ...
    **ФОРМАТ ОТВЕТА:**
    ...
    """
    retry_count = 0
    raw_model_response_content = "" # Для логирования полного ответа модели
    while retry_count < MAX_RETRIES:
        try:
            ...
            # --- НАЧАЛО ИЗМЕНЕНИЯ: ПОСТ-ОБРАБОТКА ---
            # Удаляем текст между тегами <think> и </think>
            # и также удаляем сами теги, если они в начале/конце JSON блока.
            json_str = re.sub(r"<think>.*?</think>\s*", "", raw_model_response_content, flags=re.DOTALL).strip()
            # --- КОНЕЦ ИЗМЕНЕНИЯ ---
            ...
            try:
                ...
                logger.error(f"Внутренняя ошибка декодирования JSON для {entry_id} ПОСЛЕ ОЧИСТКИ: {e_inner}. Очищенная строка: '{json_str[:500]}...'. Полный сырой ответ: '{raw_model_response_content[:500]}...'")
                raise e_inner
            ...
            logger.error(f"Ошибка декодирования JSON для {entry_id} (возможно, после очистки): {e}. Очищенная строка: '{json_str[:500] if 'json_str' in locals() else 'Не удалось получить json_str'}...'. Полный сырой ответ: '{raw_model_response_content[:500]}...'")
...
# -----------------------------------------------------------------------------
# ОСНОВНАЯ ФУНКЦИЯ ОБРАБОТКИ ДНЕВНИКА (без существенных изменений, кроме вызова обновленной extract_revolution_events)
# -----------------------------------------------------------------------------
def process_diary():
...
if __name__ == "__main__":
    logger.info("Запуск скрипта обработки дневника с ПОСТ-ОБРАБОТКОЙ <think> тегов (OpenAI-совместимый API)...")
    try:
        process_diary()
    except Exception as e:
        ...
        logger.info("Работа скрипта с ПОСТ-ОБРАБОТКОЙ <think> тегов (OpenAI-совместимый API) завершена.")