...
def extract_revolution_events(entry_id: int, text: str, date: str, extractor_model, current_knowledge_map_str: str) -> List[Dict[str, Any]]:
    ...
    **СТРОГИЕ КРИТЕРИИ ОТБОРА:**
    ...
    ВКЛЮЧАТЬ: Конкретные революционные события 1848-1849 гг., прямые реакции российского правительства, обсуждения этих событий в российском обществе, личные размышления автора об этих конкретных событиях.
    НЕ ВКЛЮЧАТЬ: Общие рассуждения о политике без привязки к 1848-1849 гг., события до 1848 или после 1849 года, внутренние российские события, не связанные с европейскими революциями, бытовые упоминания европейских стран без революционного контекста, абстрактные философские размышления, литературные или культурные события без политического контекста 1848-1849 гг.
    ДОПОЛНИТЕЛЬНАЯ ПРОВЕРКА: Перед включением каждого упоминания задай себе вопрос: "Можно ли это упоминание ПРЯМО связать с революционными событиями 1848-1849 гг. в Европе?" Если ответ "нет" или "возможно" - НЕ включай.

    Используй Карту Знаний для event_id и event_name:
    {current_knowledge_map_str}

    ФОРМАТ ОТВЕТА: Верни ТОЛЬКО JSON массив. Если нет релевантных упоминаний, верни пустой JSON массив `[]`.
    Пример объекта в массиве:
    ```json
    {{
        "entry_id": {entry_id},
        "event_id": "ID_из_Карты_Знаний или 'OTHER_1848' или null",
        "event_name": "Название события/аспекта ИЗ КАРТЫ ЗНАНИЙ или кастомное",
        "event_subtype_custom": "Уточнение для общих event_id или null",
        ...
        "brief_context": "Конкретный исторический факт (1-2 предложения).",
        "information_source": "Источник информации для автора (из текста)",
        "information_source_type": "Тип источника из списка или null.",
        "confidence": "High/Medium/Low",
        "classification_confidence": "High/Medium/Low",
        "keywords": ["список", "ключевых", "слов"],
        "text_fragment": "Точная цитата (ОДНО ИЛИ НЕСКОЛЬКО ПОЛНЫХ ПРЕДЛОЖЕНИЙ)."
    }}
    ```
    """
    retry_count = 0
...
def verify_event(event_data: Dict[str, Any], verifier_model, full_text: str, current_knowledge_map_str: str) -> Dict[str, Any]:
    manage_api_rate_limit()
    if not isinstance(event_data, dict): # Should not happen if called correctly
    ...
    while retry_count < MAX_RETRIES:
        try:
            ...
            if retry_count >= MAX_RETRIES - 1: return event_data_to_verify # Return original if all retries fail
        except Exception as e:
            retry_count += 1
    ...
    return event_data_to_verify # Should be unreachable if MAX_RETRIES > 0
    ...
    for index, row in data.iterrows():
        ...
        if os.path.exists(temp_file):
            ...
            try:
                    ...
                    if isinstance(evt_data, dict):
                        ...
                        if key not in existing_event_keys:
                            try:
                                ...
                                existing_event_keys.add(key) # Add to set to prevent duplicates within this load
                                newly_added_count += 1
                ...
                # Always save and update last processed ID, even if no new events were added from this specific temp file
                # to ensure progress if the script was interrupted after temp file creation but before main list update.
            except Exception as e:
                logger.warning(f"Ошибка при загрузке/обработке temp-файла {temp_file}: {e}. Обрабатываем запись заново.")
                # Fall through to re-process the entry if temp file loading fails badly
            else: # If temp file loaded successfully (or was empty)
                ...
                continue # Skip to next entry_id

        # Re-processing or first-time processing
        try:
            ...
            for event_item_data in extracted_events_data:
                ...
                # Ensure these are set, as verifier might accidentally remove them if not in its output schema
            ...
            with open(LAST_PROCESSED_FILE, "w") as f:
                f.write(str(current_entry_id)) # Mark as processed to avoid re-hitting the error
            time.sleep(5)
...
if __name__ == "__main__":
    ...
    try:
        process_diary()
    except Exception as e: # Catch-all for unexpected errors during setup or global scope
