...
# --- ВЕРСИЯ С УЛУЧШЕННОЙ ПОСТ-ОБРАБОТКОЙ JSON И <think> ТЕГОВ ---
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("processing.log", mode='a', encoding='utf-8'), # Изменено имя лог-файла
...
TEMP_DIR = "temp" # Изменено имя временной директории
...
Если ты используешь внутренние рассуждения, оформляй их тегами <think>...</think>. Твой конечный JSON-ответ должен быть после этих тегов или вообще без них.
"""
...
def ensure_default_values(event_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(event_dict["keywords"], list):
        event_dict["keywords"] = []
    ...
    for field, valid_values in literal_fields_map.items():
        ...
        if current_value is not None and current_value not in valid_values:
            ...
            if field in ["confidence", "classification_confidence"]:
...
# -----------------------------------------------------------------------------
# ФУНКЦИЯ ИЗВЛЕЧЕНИЯ ДАННЫХ (С УЛУЧШЕННОЙ ПОСТ-ОБРАБОТКОЙ)
# -----------------------------------------------------------------------------
    ...
    **ФОРМАТ ОТВЕТА:**
    ...
    while retry_count < MAX_RETRIES:
        try:
            ...
            raw_model_response_content = completion.choices[0].message.content

            # 1. Удаляем <think> теги
            cleaned_after_think_tags = re.sub(r"<think>.*?</think>\s*", "", raw_model_response_content, flags=re.DOTALL).strip()

            # 2. Ищем JSON внутри Markdown блоков ```json ... ```
            markdown_json_match = re.search(r"```json\s*([\s\S]*?)\s*```", cleaned_after_think_tags, flags=re.DOTALL)
            if markdown_json_match:
                json_str_for_parsing = markdown_json_match.group(1).strip()
            else:
                # 3. Если нет Markdown, ищем первый '[' или '{' и последний ']' или '}'
                # Это более устойчивый способ, чем просто удаление ```json
                # Отдаем предпочтение массиву, так как мы ожидаем массив событий.
                first_bracket = cleaned_after_think_tags.find('[')
                first_curly = cleaned_after_think_tags.find('{')
                ...
                if start_index != -1:
                    ...
                    # но это сложно сделать идеально без полного парсера.
                    # Простой поиск последнего символа может быть достаточным.
                    end_index = cleaned_after_think_tags.rfind(end_char)
                    if end_index > start_index:
                        json_str_for_parsing = cleaned_after_think_tags[start_index : end_index+1]
                    else: # Не нашли подходящий конец, берем все после начала
                        json_str_for_parsing = cleaned_after_think_tags[start_index:]
                else: # Не нашли ни '[', ни '{', используем всю строку после удаления <think>
                    json_str_for_parsing = cleaned_after_think_tags

            try:
                events = json.loads(json_str_for_parsing.strip())
            except json.JSONDecodeError as e_inner:
                logger.error(f"Ошибка декодирования JSON для {entry_id} ПОСЛЕ ОЧИСТКИ: {e_inner}. Строка для парсинга: '{json_str_for_parsing[:500]}...'. Полный сырой ответ: '{raw_model_response_content[:500]}...'")
                raise e_inner

            if not isinstance(events, list):
                if isinstance(events, dict) and not events: # пустой объект {}
                     events = []
                elif isinstance(events, dict): # Если модель вернула один объект
                     logger.warning(f"Экстрактор вернул объект вместо списка для {entry_id}. Оборачиваем в список. Ответ: {events}")
                     events = [events]
                else:
                    logger.error(f"Экстрактор вернул не список и не объект для {entry_id}. Тип: {type(events)}. Ответ: {events}")
                    raise TypeError("LLM returned non-list/non-dict for event extraction")

            return events

        except json.JSONDecodeError as e:
            logger.error(f"Ошибка декодирования JSON для {entry_id} (возможно, после очистки): {e}. Строка для парсинга: '{json_str_for_parsing[:500] if json_str_for_parsing else 'EMPTY' }...'. Полный сырой ответ: '{raw_model_response_content[:500]}...'")
            if retry_count >= MAX_RETRIES -1: raise
        except Exception as e:
            retry_count += 1
            logger.warning(f"Ошибка экстракции для {entry_id} (попытка {retry_count}/{MAX_RETRIES}): {str(e)}. Полный сырой ответ: '{raw_model_response_content[:500]}...'")
            if retry_count < MAX_RETRIES:
                time.sleep(RETRY_WAIT_BASE * (retry_count + random.uniform(0,1)))
            else:
                logger.error(f"Превышены попытки экстракции для {entry_id}. Пропускаем. Полный сырой ответ: '{raw_model_response_content[:500]}...'")
                return []
    return []
...
def process_diary():
    ...
    for dir_path_str in [TEMP_DIR, os.path.dirname(TEMP_RESULTS_FILE), os.path.dirname(FINAL_RESULTS_FILE)]:
        # Проверка, что путь не пустой перед созданием
        if dir_path_str: # Пустая строка os.path.dirname("") вернет текущую директорию
            dir_path = os.path.normpath(dir_path_str) # Нормализуем путь
            # Дополнительная проверка, что мы не пытаемся создать корень диска или что-то подобное
            if dir_path and dir_path != os.path.abspath(os.sep):
                 if not os.path.exists(dir_path):
                    try:
                        ...
                        return
            elif not dir_path: # Если dirname вернул пустую строку (например, для файла в текущей директории)
                pass # Ничего не делаем, директорию создавать не нужно
    ...
    if os.path.exists(TEMP_RESULTS_FILE):
        try:
            with open(TEMP_RESULTS_FILE, "r", encoding="utf-8") as f:
                # Проверка, что файл не пустой перед загрузкой JSON
                file_content_temp = f.read()
                if file_content_temp.strip():
                    all_events = json.loads(file_content_temp)
                    logger.info(f"Загружено {len(all_events)} уже обработанных событий из {TEMP_RESULTS_FILE}.")
                else:
                    logger.info(f"Файл {TEMP_RESULTS_FILE} пуст. Начинаем с нуля.")
                    all_events = []
    ...
    for index, row in data.iterrows():
        ...
        if os.path.exists(temp_file_path):
            ...
            try:
                with open(temp_file_path, "r", encoding="utf-8") as f_temp_entry:
                    file_content_entry_temp = f_temp_entry.read()
                    if file_content_entry_temp.strip():
                        entry_events_from_temp = json.loads(file_content_entry_temp)
                    else:
                        entry_events_from_temp = [] # Если файл пуст, считаем, что событий нет

                # Удаляем ранее добавленные события для этой entry_id, чтобы избежать дубликатов при перезаписи temp
                all_events = [evt for evt in all_events if isinstance(evt, dict) and evt.get('entry_id') != current_entry_id]
                ...
                if newly_added_count > 0 or not entry_events_from_temp: # Если были добавлены или если temp был пуст (что тоже результат)
                    logger.info(f"Добавлено/загружено из temp {newly_added_count} событий для {current_entry_id}.")

            except Exception as e:
                logger.warning(f"Ошибка загрузки temp-файла {temp_file_path}: {e}. Обрабатываем заново.")
                # Не продолжаем, а идем на новую обработку (удаляем continue)
            else:
                try:
        ...
        try:
            ...
            # Удаляем старые события для этой entry_id перед добавлением новых, если они были
            all_events = [evt for evt in all_events if isinstance(evt, dict) and evt.get('entry_id') != current_entry_id]
