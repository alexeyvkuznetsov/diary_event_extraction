...
def extract_revolution_events(entry_id: int, text: str, date: str, extractor_model, current_knowledge_map_str: str) -> List[Dict[str, Any]]:
    ...
    # current_knowledge_map_str будет глобальной переменной knowledge_map_for_prompt
    prompt = f"""
...
def verify_event(event_data: Dict[str, Any], verifier_model, full_text: str, current_knowledge_map_str: str) -> Dict[str, Any]:
    ...
    # Формируем "пользовательский" промпт для верификатора
    ...
    **Формат ответа:**
    ...
    while retry_count < MAX_RETRIES:
        try:
            response = verifier_model.generate_content( # Модель verifier_model уже имеет system_instruction (VERIFIER_SYSTEM_PROMPT_STATIC)
                user_prompt_for_verifier,             # Передаем динамическую часть здесь
                safety_settings=SAFETY_SETTINGS,
                generation_config=GenerationConfig(
                    temperature=0.5,
                    response_mime_type="application/json"
...
def process_diary():
    ...
    for index, row in data.iterrows():
        ...
        try:
            # Используем глобальную knowledge_map_for_prompt для extract_revolution_events
            ...
            for event_item_data in extracted_events_data:
                ...
                # Используем глобальную knowledge_map_for_prompt для verify_event
