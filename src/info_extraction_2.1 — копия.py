...
def extract_revolution_events(entry_id: int, text: str, date: str, extractor_model, current_knowledge_map_str: str) -> List[Dict[str, Any]]:
    ...
    *   Выбери ОДНО из следующих ТОЧНЫХ значений: "{'Официальные источники (газеты, манифесты)', 'Неофициальные сведения (слухи, разговоры в обществе)', 'Личные наблюдения и опыт автора', 'Информация от конкретного лица (именованный источник)', 'Информация из нескольких источников', 'Источник неясен/не указан'}".
    *   **Используй "Информация из нескольких источников" ТОЛЬКО В ТОМ СЛУЧАЕ, если автор явно указывает или из контекста четко следует, что его знание/мнение о КОНКРЕТНОМ СОБЫТИИ сформировано на основе информации, полученной из ДВУХ И БОЛЕЕ РАЗНЫХ ИСТОЧНИКОВ (например, "прочитал в газете и слышал от N.", или "по слухам и официальным сообщениям"). Не используй эту категорию, если автор просто перечисляет разные новости из разных источников о разных событиях в одной записи.**
...
def verify_event(event_data: Dict[str, Any], verifier_model, full_text: str, current_knowledge_map_str: str) -> Dict[str, Any]:
    ...
    *   Проверь `information_source`: должно описывать, откуда автор дневника узнал о событии, на основе текста. Если не указан, оставь "Источник неясен/не указан".
    *   Проверь `information_source_type`: должно быть одним из следующих **точных** русских значений: "Официальные источники (газеты, манифесты)", "Неофициальные сведения (слухи, разговоры в обществе)", "Личные наблюдения и опыт автора", "Информация от конкретного лица (именованный источник)", "Информация из нескольких источников", "Источник неясен/не указан".
        *   **Критерий для "Информация из нескольких источников":** эта категория применяется, если автор явно или по контексту указывает, что его знание/мнение о ДАННОМ КОНКРЕТНОМ СОБЫТИИ основано на комбинации ДВУХ И БОЛЕЕ РАЗНЫХ ИСТОЧНИКОВ. Не путать с упоминанием разных источников для разных событий.
        *   Если не определить или значение неверное, установи `null`.
